...
cjs.paired.fn=function(detects.in,L.in=NULL,seeds.in=NULL,se.out=F,d=NULL,bootstrap=F,common.start.in=0){
	...
	for(g in 1:2){
		...
		# print("cjs.paired.fn/p(censored)")
...
#browser()
	...
	paired.cjs.out = optim(par=unlist(seeds.in), fn=paired.cjs.lik, counts.in = counts.in, num.period=num.period, use.hist=obs.hist,L=L.in,d.in=d,
		common.start=as.numeric(common.start.in),like.check=F, method = "BFGS", hessian = F, control = list(trace=F,parscale = rep(parscale.in, 2*(num.period * 2 - 1)), 
...
#print("Done with paired cjs w equal params")		
	...
	if(se.out){
		...
		like.check=F,method = "BFGS", hessian = T, control = list(parscale = rep(parscale.in, 2*(num.period * 2 - 1)), reltol = reltol.in,maxit=maxit.in,trace=1,
		ndeps=rep(parscale.in, 2*(num.period * 2 - 1))))
