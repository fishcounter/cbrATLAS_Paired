...
#	print(c(mean(activetime.matrix[,3],na.rm=T),mean(activetime.matrix[,4],na.rm=T)))
 ...
 # this code is modified to use all covariances beyond adjacent detection points (differs slightly in coding)
 
   for(i in 2:(num.period-1)){
#	cov.LiLj=0
#	varL.ratio=0
	for(j in (i-1):num.period){
      cov.LiLj[j]=(1-est.p[i-1])*(1-est.p[j])*(L.cov2[i-1,i]-L.cov1[i-1,i])+L.cov1[i-1,i]
