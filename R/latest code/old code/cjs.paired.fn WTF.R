...
# if common.start.in is > 0, set lower release to equal upper release starting at first common site to end
	...
	params=paired.cjs.out$par;print(params);print("First Run")
	if(common.start.in>0){
		common.param=c(seq(1,(length(params)-1),2),seq(2,(length(params)-1),2),length(params))
		print("init common params");print(common.param)

print("back into cjs.paired.fn");print(paired.cjs.out$par)		
print("set common estimates");print(paired.cjs.out$par[c(1:(length(paired.cjs.out$par)/2))][!(common.param<(common.start.in+1))])
#
...
print("made it to correction")
	...
	#print(cjs.out$par)
	if(se.out){
	print("estimate the unestimatable!")
...
#	urps here:		paired.cjs.param[dim(paired.cjs.param)[1]/2+c(1:(dim(paired.cjs.param)[1]/2)),][!(common.param<(common.start.in+1)),]=
...
s2=function(s1,s2,se1,se2,cov1=0) {
 ### estimate two reach survival
 ### s*: survivals; se*: standard errors

 out=round(s1*s2,4)
 out.var=((se1*s2)^2)+((se2*s1)^2)+2*cov1*s1*s2
 out.sqrt=round(sqrt(out.var),4)
 print.out=paste(c(out," (",out.sqrt,")"),collapse="")
 return(print.out)
 }
