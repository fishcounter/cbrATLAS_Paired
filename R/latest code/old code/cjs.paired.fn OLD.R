...
print("going into paired.cjs.lik!")
   paired.cjs.out = optim(par=unlist(seeds.in), fn=paired.cjs.lik, 
	...
	# reset estimates to match final parameters used in likelihood 
	print("confirm out put from 1st run")
	print(paired.cjs.out$par)

	if(!is.null(fx.1)){
		paired.cjs.est=rep(0,length(par.names)) # as defined line 102 above
		names(paired.cjs.est)=par.names
		paired.cjs.est[par.names %in% fx.1]=1

		for(i in c(1:length(paired.cjs.out$par))){paired.cjs.est[match(names(paired.cjs.out$par[i]),names(paired.cjs.est))]=paired.cjs.out$par[i]}
		paired.cjs.out=as.data.frame(paired.cjs.out)
		}
		
browser()
	
print(paired.cjs.est$par)


 	if(common.start.in>0){
		params=paired.cjs.est$par
...
cat("\n cjs.paired.fn_rab: estimate Hessian for CJS estimates for paired release \n")    
		...
		}else{ print("why the heck is it popping up?")
