...
#' @export
#'
paired.cjs.lik=function(params.in,counts.in,num.period,use.hist,L.in=NULL,d.in=NULL,common.start=NULL,like.check=F,hessian=F){
  ...
  # helper fn
  	bounding.fn=function(x){
		...
		if((!(x<0)) & (!(x>1))){out=x}
		return(out)}
		if(restrict.params){seeds.in=logit.fn(seeds.in)} # logitize input
...
if(common.start>0){print("after resetting matching params")
print(params)  }
#  print("input to paired.cjs.lik")
#  params=list(params.in[1:(length(params.in)/2)],params.in[((length(params.in)/2)+1):length(params.in)])
